...
重新定义了一套快捷键方案，并充分发掘鼠标侧键的潜力。又能作为一个高效启动器，最后还良好支持热字符。

为获得最佳体验，强烈建议搭配带侧键鼠标和全局鼠标手势软件，并将捷键设置为开机自启。
...
CapsLock + 单按键能激活某些预设功能，其中部分键位参考了 [MyKeymap][]，并从苹果 macOS 系统快捷键得到启发。且尽量保证单手操作就能完成，例如 CapsLock + f 实现了最大化窗口。

| 搭配按键 | 用途 |
| ---- | ---- |
| 鼠标左键 | 移动窗口 |
| Q | 关闭窗口(参考 macOS command +（q）uit) |
| W | 宽度充满屏幕 |
| E | 切换到上个窗口 |
| R | 在当前程序的窗口轮换 |
...
| Z | 复制文件路径或纯文本 |
| V | 窗口移到下一个显示器 |
| M | 窗口最小化(参考 macOS command +（m）inimize) |
...
| 滚轮上滑 | - | - | - | - | - | - | - | - | 调高音量 | 调高音量 | 下一曲 | - |
| 滚轮下滑 | - | - | - | - | - | - | - | - | 调低音量 | 调低音量 | 上一曲 | - |
| **鼠标左侧后退键**(XB1) | - | - | 关闭/close | 关闭窗口 | 关闭标签 | 关闭窗口 | 关闭窗口 | 关闭窗口 | 下一曲 | 下一曲 | - | - |
