...
转换器统一对 BaseEnum 进行转换，对于每个枚举类型，可通过返回的keys来自定义转换的方式。
...
开始使用。注意这里的 DocxEnum 用法。

```java
@PostMapping("/arrangement")
public Object importData(final MultipartFile[] file,
                            @RequestParam(name = "type") final DocxEnum docxEnum)
        throws IOException {
            ...
        }
```
