...
在这种方式下，`function (x) { ... }`是一个**匿名函数**，它没有函数名。但是，这个匿名函数赋值给了变量
...
`arguments`，并且循环要从索引`2`开始以便排除前两个参数，这种写法很别扭，只是为了获得额外的
`rest` 参数，有没有更好的方法？
...
`a`、`b`，多余的参数以数组形式交给变量 `rest`，所以，不再需要`arguments`
我们就获取了全部参数。
...
或者`apply()`调用箭头函数时，无法对`this`进行绑定，即传入的第一个参数被忽略。
...
生成器（Generator）是 ES6 引入的一种**特殊的可暂停函数**，通过 `function*`
...
Redux-Saga
用生成器实现**可测试的、声明式的副作用管理**，是生成器在业界的标杆应用。
...
[简介 - JavaScript 教程](https://liaoxuefeng.com/books/javascript/introduction/index.html) -
廖雪峰的官方网站
