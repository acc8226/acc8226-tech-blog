---
title: Vue2 Class 与 Style 绑定
date: 2026-06-24 20:35:14
updated: 2026-06-24 20:35:14
categories:
  - 框架
  - js
tags:
...
上面的语法表示 `active` 这个 class 存在与否将取决于数据 property `isActive` 的 [truthiness](https://developer.mozilla.org/zh-CN/docs/Glossary/Truthy)。<!-- more -->
...
> 这个章节假设你已经对 [Vue 组件](https://v2.cn.vuejs.org/v2/guide/components.html)有一定的了解。当然你也可以先跳过这里，稍后再回过头来看。
...
这样写只会**渲染数组中最后一个被浏览器支持的值**(也就是从后往前匹配)。在本例中，如果浏览器支持不带浏览器前缀的 flexbox，那么就只会渲染 `display: flex`。
