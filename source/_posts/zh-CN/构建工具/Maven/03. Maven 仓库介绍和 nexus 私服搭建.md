...
默认情况下，无论是 Linux 还是 Windows，每个用户在自己的用户目录下都有一个路径名为 `.m2/respository/` 的仓库目录。
...
nexus 私服实际上是一个 javaEE 的 web 系统。用来管理一个公司所有的 jar 包,实现项目 jar 包的版本统一。

jar 下载搜索顺序：本地仓库 -->  nexus 私服 或者 Maven中央库.
