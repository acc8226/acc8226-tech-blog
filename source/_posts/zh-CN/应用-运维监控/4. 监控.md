...
| *Proxy mode* |     | 选择proxy模式。<br>**主动** - proxy将连接到Zabbix server并请求配置数据<br>**被动** - - Zabbix server连接到proxy<br>*Note* 没有加密的通信(敏感的)proxy配置数据可能会成为可以访问Zabbix服务器的端口时，使用一个主动的proxy。这是可能的，因为任何人都可以假装是一个活动的proxy并请求配置数据，如果身份验证没有发生或proxy地址不受限制在*Proxy 地址*字段。 |
| *Proxy address* |     | 如果指定了，那么主动的proxy请求只接受这个以逗号分隔的IP地址列表(可选的CIDR标记)，或者主动 Zabbix proxy的DNS名称。<br>该字段仅在“*Proxy 模式*”字段中选择了active proxy时有效。 宏是不支持的。<br>从Zabbix 4.0.0开始就支持这个选项。 |
