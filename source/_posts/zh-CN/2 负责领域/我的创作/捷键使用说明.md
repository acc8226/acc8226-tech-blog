...
Caps + C 键打开音量面板, 如果同时插了耳机和音响，在面板中按 R 键切换默认音频输出。在面板中按 V 键打开「 音量设置 」, 此处能调整各应用音量。
