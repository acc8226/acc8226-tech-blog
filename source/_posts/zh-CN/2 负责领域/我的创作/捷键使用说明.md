...
当我开发几个月后，发现我列举的功能市面上已有此类软件，捷键的完成度也比较低。但这毕竟是我闲暇时开发。目前看来 BetterAndBetter 和捷键是比较像的，捷键的优点则是杂糅了各家之长+小巧。缺点就是用户交互上不方便，下一步我的工作会逐渐放在提升交互。目标是对标 BetterAndBetter for macOS，争做 windows 平台必备的键鼠辅助神器。
...
| 【低层次】热字符串拓展 | [Espanso][]【免费】| autokey 脚本或 [Espanso][]【免费】软件 | 基于 ahk 的 [捷键][]【免费】或 [Espanso][]|
...
| 【低层次】启动程序/窗口切换 | [Hammerspoon][] 脚本【免费】| / | 基于 ahk 的 [捷键][]【免费】或 MyKeymap|
...
注：音量面板组件源于[咸鱼阿康](https://github.com/xianyukang)的开源项目 [MyKeymap](https://github.com/xianyukang/MyKeymap)，并由我做了键鼠增强。在召唤面板后的基础上可通过鼠标滑动来调节音量。
...
拓展了 Printscreen、ScrLk 和 Pause 按键的用法。默认功能分别改写为音量减少、暂停播放、音量增加。

#### 数字小键盘额外支持

数字开关关闭情况下，4（←）、5（←）、 6（→） 按键分别为音量减少、暂停播放、音量增加。8（↑）、2（↓）分别为上一曲和下一曲。

#### 程序员按键特定支持

为程序员朋友添加乐 ctrl + alt + t 为在资源管理器中打开终端
...
| ! \\ = | <span style="color: red;">G</span>an叹号 | <span style="color: red;">Z</span>huan义符 | <span style="color: red;">D</span>eng号 |
| \_ % # | <span style="color: red;">X</span>ia划线 | <span style="color: red;">B</span>ai分号 | <span style="color: red;">W</span>a井 |
...
* F7 激活或关闭该功能，会穿透当前窗口
* F8 当已激活状态下，切换反转设置
* F9 当已激活状态下，切换暂停。暂停后，孔不会跟随鼠标
* Esc 当已激活状态下关闭该功能

使用方法和使用效果：
想要从桌面拿一个文件到 word，无需暂时最小化，按下 F7 后直接穿透到下层，如果下层是资源管理器或者桌面，直接托放进去即可。
工作中要查资料，偷偷按下 F7，透过小洞看看网页应该没人知道。
...
```ini
[Common]
; 0 通过 alt + 空格启动 anyrun 组件，否则 1 表示通过 ctrl + 空格启动
AnyRunUseCtrl=0
...
优先建议搭配带侧边按键的鼠标 和 鼠标手势驱动来驱动快捷键，例如【付费】[WGestures 2][WGestures 2付费链接]。最后才是考虑快捷键，其实我也做了限制，部分按键为 F13 以上功能键，你们的键盘上没有的话无法直接驱动。
