...
要求：案例分析是每题 15 分，共计 5 题目，共 75 分。得分 45 分以上合格。
...
52-53\. 给定关系 R（U, F），其中 U = {A, B, C, D, E, H}, F = {A→B, B→DH, A→H, C→E}，关系有 （52），F 中 （53）。
...
1. 一定要熟悉题目的出法，熟悉所有的必要的概念，包括用例图、ER 图、数据流图的各元素构成。
1. 要熟悉各类题型的解答方法。
1. 试卷至少做 1 套，然后实战 1 套。做过的试卷再重做 1 遍，这样 2 套试题就算一共做了 4 次。从考中学，从考中试题提取出以后这种问题应该怎么答。
1. 目的是为了实战，可以借助 ai 快速获取自己想要的知识点。UML 和 ER 图真的很重要。
1. 只要基础好，中级得到 60% 的分数还是不难的，要知道考试考分是如何判定的，然后去“迎合”这种方向，同时通过软考去顺便提升自己的专业知识。
1. 对我而言数据结构和算法题偏难，以后我要加强数据结构和算法。
1. 考试中要学会对时间进行取舍，简单的题可累计时间，中等的题尽量花了时间并能得分，复杂的题可以适当放弃。
1. 以后时间要用对地方，要高效学习，并不是堆积大量时间就能通过考试。花了大量时间也不一定能取得好成绩。
1. 考试用的画图软件要提前熟悉。对我而言，我会接下来后完善笔记，尽量学习 mermaid 语法去画图。

路线要提前规划好四点：**住的地方** -> 交通方式 -> **住酒店** -> 交通方式 -> **考点** -> 交通方式 -> **回家**

提前订好酒店，如有必要提前规划好交通线路和安排好食宿。
